...
public class DataFinshOrder implements Parcelable {
    @SerializedName("id")
    @Expose
    private Integer id;
    ...
    protected DataFinshOrder(Parcel in) {
        if (in.readByte() == 0) {
            id = null;
        } else {
            id = in.readInt();
    ...
    public static final Creator<DataFinshOrder> CREATOR = new Creator<DataFinshOrder>() {
        @Override
        public DataFinshOrder createFromParcel(Parcel in) {
            return new DataFinshOrder(in);
        }

        @Override
        public DataFinshOrder[] newArray(int size) {
            return new DataFinshOrder[size];
        }
    };
